...
#фактически, этот get-запрос динамически генерирует веб-страницы при помощи контента из таблиц Posts и Comments
	...
	@count = (@db.execute 'select count(*) from Comments where post_id = ?', [@post_id])[0][0] #определяем переменную @count, в которую передаётся результат sql-запроса
	#этот sql-запрос возвращает количество комментариев, у которых post_id равно значению глобальной переменной @post_id
	#так как данные возвращаются в виде массива, в котором находится один элемент - хэш, то сначала выводится на экран нулевой элемент массива, а затем - запись из хэша с ключом 0
