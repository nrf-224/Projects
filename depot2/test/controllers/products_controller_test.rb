require 'test_helper'
#в данном файле хранятся интеграционные тесты для контроллера products. выполняются при помощи команды rails test
...
class ProductsControllerTest < ActionDispatch::IntegrationTest #набор тестов для контроллера Products наследуется из класс integrationTest в модуле ActionDispatch
  ...
  test "should get index" do #определяем тест с названием should get index, который возвращает ссылку на список продуктов
    get products_url #get-запрос к index.html.erb
